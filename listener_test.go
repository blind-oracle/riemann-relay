...
func getTestListener(ch chan []*Event) (l *listener, addr string, err error) {
	for port := 0; port < 50; port++ {
		addr = "127.0.0.1:" + strconv.Itoa(rand.Intn(20000)+20000)
		if l, err = newListener([]string{addr}, 1*time.Second, ch); err == nil {
			return
		}
	}
